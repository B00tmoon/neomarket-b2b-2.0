...
async def create_product(
    ...
    """
    Create a new product (US-B2B-01).

    seller_id is extracted from JWT claims / request.state.user / X-Seller-Id —
    never from request body (IDOR prevention).
    Requires: title, category_id, at least one image.
    On success: status=CREATED, skus=[] (unless provided), HTTP 201.
    All ids are UUID strings in the response.
    """
